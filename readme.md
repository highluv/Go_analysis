...
### Три формы данных
Данные проходят три стадии, и это разделение — главный организующий принцип (пакеты
`model` → `normalize` → `analyze`):

| Форма | Что это | Где живёт |
|-------|---------|-----------|
| **raw** | исходные манифесты как пришли из API (JSON) + метаданные | blob-хранилище (S3/MinIO) + строка в БД |
| **normalized** | внутренние сущности (`Workload`, `Service`, `AuthorizationPolicy`…) | JSONB в БД |
| **derived** | результат: `AllowedEdge` + `Evidence` | реляционные таблицы |

**Зачем так (tradeoff).** Храня raw отдельно, мы можем переанализировать старый снапшот новой
версией движка, не ходя в кластер повторно. Плата — дублирование данных и необходимость
следить за консистентностью трёх представлений. Для аудита воспроизводимость важнее экономии
места, поэтому выбор в её пользу.
